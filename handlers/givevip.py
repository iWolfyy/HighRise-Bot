    ...
    try:
        ...
        if not match:
            await bot.highrise.send_whisper(
                user.id, "❌ Your user info could not be found.")
            return
        _, _, _, _, is_owner = match

        if not is_owner:
            await bot.highrise.send_whisper(
                user.id, "❌ You need to be the Room Owner to use !givevip.")
            return
        ...
        await bot.highrise.send_whisper(
            user.id, f"✅ @{target_user.username} has been granted VIP status.")
        await bot.highrise.send_whisper(target_user.id,
                                        "🌟 You have been granted VIP status!")

    except Exception as e:
        await bot.highrise.send_whisper(user.id,
                                        f"❌ Error processing !givevip: {e}")
